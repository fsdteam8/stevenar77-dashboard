...
import { Pencil, CheckCircle, Eye, Trash2, Loader2 } from "lucide-react";
...
export default function TemplateTable({ templates }: Props) {
  ...
  //  Sync incoming templates with local state
  useEffect(() => {
    setLocalTemplates(templates);
  }, [templates]);

  //  Activate or deactivate template
  const handleActivate = async (tpl: TemplateResponse) => {
    if (!tpl._id) return;

    // Toggle between "active" and "deactive"
    const newStatus = tpl.status === "active" ? "deactive" : "active";
      ...
      <Dialog open={openPreview} onOpenChange={setOpenPreview}>
        <DialogContent className="max-w-3xl p-0 overflow-hidden rounded-xl shadow-xl bg-white">
          ...
          <div className="bg-gray-50 p-4 flex justify-end border-t border-gray-200">
            <Button variant="outline" onClick={() => setOpenPreview(false)} className=" cursor-pointer">
              Close
            </Button>
          </div>
        </DialogContent>
